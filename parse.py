...
bundle_dims = [
                  "npscalar__mod__cparam",
                  "nchemspec__mod__cparam",
                  "mreactions__mod__chemistry",
                  "ndustspec__mod__cparam",
                  "n_forcing_cont_max__mod__cparam",
                  "npscalar",
                  "nchemspec",
                  "ndustspec",
                  "n_forcing",
...
def is_arithmetic_expression(val):
     if "**" in val: 
       return False
     return is_float(val.replace("/","").replace("+","").replace("-","").replace("*",""))
...
def gen_field3(index):
    return f"(Field3){{Field({index}-1), Field({index}), Field({index}+1)}}"
def gen_field(index):
...
def map_dot_mn(func_call):
    ...
    ladd_param = params[-1].split("=")[-1].strip()
    conditional = f"if({ladd_param}) then"
    body = f"{params[2]} = dot({params[0]},{params[1]})"
    second_body = f"{params[2]} = {params[2]} + dot({params[0]},{params[1]})"
    return [conditional,"else",body,second_body]

def map_del2_main(func_call):
    params = func_call["parameters"]
    return [f"{params[2]} = laplace({gen_field(params[1])})"]
...
def map_del2_other(func_call):
    ...
    if first_param_name in func_call["static_variables"] and func_call["static_variables"][first_param_name]["profile_type"] == "vtxbuf":
      indexes =get_indexes(params[0],first_param_name,0)
      if len(indexes) == 0:
          return [f"{params[1]} = laplace({first_param_name})"]

    if (first_param_name in func_call["static_variables"] and func_call["static_variables"][first_param_name]["profile_type"] == "vtxbuf_bundle"): 
      indexes =get_indexes(params[0],first_param_name,0)
      if len(indexes) == 4 and indexes[:-1] == [":",":",":"]:
         return [f"{params[1]} = laplace({first_param_name}[{indexes[3]}])"]
    pexit("PARAMS ",params)
...
def map_del4v(func_call):
    params = func_call["parameters"]
    return [f"{params[2]} = del4({gen_field3(params[1])})"]
...
def map_traceless_strain(func_call):
    ...
    if len(params)==5 and all(x[-1] is None for x in params):
        ...
        if params[4][0]==".false.":
            ...
            add_line_1 = f"{params[2][0]}(:,1,2) = {params[2][0]}(:,1,2) + sshear__mod__cdata"
            add_line_2 = f"{params[2][0]}(:,2,1) = {params[2][0]}(:,2,1) + sshear__mod__cdata"
            end_if_line = "endif"
            res.append(if_line)
...
def map_grad_main(func_call):
    params = func_call["parameters"]
    return [f"{params[2]} = gradient({gen_field(params[1])})"]
def map_grad_other(func_call):
    params = func_call["parameters"]
    first_param_name = params[0].split("(")[0]
    if (params[0] in func_call["static_variables"] and func_call["static_variables"][params[0]]["profile_type"] == "vtxbuf"):
      return [f"{params[1]} = gradient({params[0]})"]
...
def map_del2v_etc(func_call):
    ...
    for i in range(2,len(params)):
        ...
        name = names[i]
        if param[-1] == "del2":
            res.append(f"{param[5]} = laplace({gen_field3(names[1])})")
        elif param[-1] == "graddiv":
            if names[1].strip()  == "":
                pexit("WHAT TO DO? ",params)
            res.append(f"{param[5]} = gradient_of_divergence({gen_field3(names[1])})")
        elif param[-1] == "curlcurl":
            res.append(f"{param[5]} = curlcurl({gen_field3(names[1])})")
        else:
            pexit("WHAT TO DO? ",param)
    #print("PARAMS: ",params)
    #pexit("RES: ",res)
    return res
...
def map_d2f_dxj(func_call):
    params = func_call["parameters"]
    return [f"{params[2]} = d2f_dxj({gen_field(params[1])})"]
def map_g2ij(func_call):
    params = func_call["parameters"]
    return [f"{params[2]} = hessian({gen_field(params[1])})"]
...
def map_der_main(func_call):
    ...
    j = int(params[3][0])
    if j == 1:
        return [f"{names[2]} = derx({gen_field(names[1])})"]
    elif j == 2:
        return [f"{names[2]} = dery({gen_field(names[1])})"]
    elif j == 3:
        return [f"{names[2]} = derz({gen_field(names[1])})"]
    print("j=",j)
    pexit("don't know whic der to call")
...
def map_smooth_mn(func_call):
    params = func_call["new_param_list"]
    names  = func_call["parameters"]
    return [f"{names[3]} = gaussian_smooth({gen_field(names[1])})"]
    pexit("What to do?")
...
def map_der2_main(func_call):
    ...
    if j == 1:
        return [f"{names[2]} = derxx({gen_field(names[1])})"]
    elif j == 2:
        return [f"{names[2]} = deryy({gen_field(names[1])})"]
    elif j == 3:
        return [f"{names[2]} = derzz({gen_field(names[1])})"]
    print("j=",j)
...
def map_der5(func_call):
    ...
    names = func_call["parameters"]
    if j == 1:
        return [f"{names[2]} = der5x{ignoredx_str}({gen_field(names[1])})"]
    elif j == 2:
        return [f"{names[2]} = der5y{ignoredx_str}({gen_field(names[1])})"]
    elif j == 3:
        return [f"{names[2]} = der5z{ignoredx_str}({gen_field(names[1])})"]
    print("j=",j)
    pexit("don't know whic der6 to call")
...
def map_del4graddiv(func_call):
    ...
    #print(params)
    #TODO write
    return ['not_implemented("del4graddiv")']
    ##pexit("what to do?\n")

def map_del6fj(func_call):
    params = func_call["parameters"]
    return [f"{params[3]}  = del6fj({gen_field(params[2])}, {params[1]})"]
...
def map_der5i1j(func_call):
    ...
    if i == 1 and j == 1:
        return [f"{names[2]} = der6x({gen_field(names[1])})"]
    elif i == 2 and j == 2:
        return [f"{names[2]} = der6y({gen_field(names[1])})"]
    elif i == 3 and j == 3:
        return [f"{names[2]} = der6z({gen_field(names[1])})"]
    else:
        first_der_name  = index_to_der_name(i)
        second_der_name  = index_to_der_name(j)
        return [f"{names[2]} = der5{first_der_name}1{second_der_name}({gen_field(names[1])})"]
    #print(params)
    pexit("what to do?")
def map_der2i2j2k(func_call):
    names = func_call["parameters"]
    return [f"{names[2]} = der2i2j2k({gen_field(names[1])})"]
...
def map_der4i2j(func_call):
    ...
    if i == 1 and j == 1:
        ...
        first_der_name  = index_to_der_name(i)
        second_der_name  = index_to_der_name(j)
        return [f"{names[2]} = der4{first_der_name}2{second_der_name}({gen_field(names[1])})"]
    pexit("what to do?")

def map_multmm_sc_mn(func_call):
    params = func_call["parameters"]
    return [f"{params[2]} = contract({params[0]},{params[1]})"]
...
def map_remove(func_call):
    return []

def map_epsilon(func_call):
    pexit("HI")

def return_false(func_call):
    return [".false."]
...
sub_funcs = {
    ...
    "u_grad_kurganov_tadmore":
    {
      ...
      "map_func": map_u_grad_kurganov_tadmore
    },
    "cross_mixed":
    {
        "output_params_indexes": [2],
    ...
    "set_zero_below_threshold":
    {
      "output_param_indexes": [0],
      "map_func": map_directly
    },
    "tini_sqrt_div_separate":
    {
      "output_param_indexes": [0],
      "map_func": map_directly
    },
...
mk_param_lines = [
    "logical, parameter, dimension(npencils__mod__cparam):: lpenc_required  = .false.",
    "logical,            dimension(npencils__mod__cparam):: lpenc_diagnos   = .false.",
    "logical,            dimension(npencils__mod__cparam):: lpenc_diagnos2d = .false.",
    "logical,            dimension(npencils__mod__cparam):: lpenc_video     = .false.",
    "logical,            dimension(npencils__mod__cparam):: lpenc_requested = .false.",
    "logical,            dimension(npencils__mod__cparam):: lpencil         = .false."
]
...
def fix_scientific_notation(line):
    return re.sub(r'(\d+)\.(\d?)[Dd]([+-]?\d+)',r'\1.\2e\3',line)
...
class Parser:
    ...
    def __init__(self, files,config):
        ...
        self.static_writes = []
        self.static_variables_to_declare = []
    ...
    def get_vtxbuf_name_from_index_base(self,prefix, index):
        index_map = {
                ...
                "ind__mod__cdata": "DUST_DENSITY",
                "ilnnd__mod__cdata": "DUST_DENSITY",
                "ireaci__mod__chemistry": "CHEMISTRY_REACTIONS",
                "ichemspec__mod__cdata": "CHEMISTRY_SPECIES",
        }
        ...
        if ":" in index and  "iudx__mod__cdata"  in index  and "iudz__mod__cdata" in index:
            ...
            return f"{prefix}DUST_VELOCITY[{bundle_index}-1]"
        elif "iudx__mod__cdata" in index:
            bundle_index = index.split("(")[-1].split(")")[0]
            return f"{prefix}DUST_VELOCITY[{bundle_index}-1].x"
        elif "iudy__mod__cdata" in index:
            bundle_index = index.split("(")[-1].split(")")[0]
            return f"{prefix}DUST_VELOCITY[{bundle_index}-1].y"
        elif "iudz__mod__cdata" in index:
            bundle_index = index.split("(")[-1].split(")")[0]
            return f"{prefix}DUST_VELOCITY[{bundle_index}-1].z"
        if ":" not in index and any([var in index for var in index_map]):
            bundle_index = index.split("(")[-1].split(")")[0]
            base = index.split("(")[0].strip()
            infix = index_map[base]
            if bundle_index == base and base in self.static_variables and len(self.static_variables[base]["dims"]) == 1:
                return f"{prefix}{infix}"
            return f"{prefix}{infix}[{bundle_index}-1]"
        ...
        if "(" in index:
            ...
            index = index.split("(",1)[1].split(")")[0].strip()
            if index == "1":
                vec_index = "x"
            elif index == "2":
                vec_index = "y"
    ...
    def update_static_var_dims(self):
      ...
      #TP don't to parameter unfolding for now simply replace with internal names
    ...
    def update_static_var_value(self,var):
        #TP for now simple change to mod name
    ...
    def get_static_passed_parameters(self,parameters,local_variables,local_module_variables):
        ...
        for i,parameter in enumerate(res):
            ...
            #TP: have to do this in a stupid manner i.e. info[4] == info[6] for backwards compability: TODO fix
            if len(parameter[0].split("=")) == 2:
                param_name,passed_param = [part.strip().lower() for part in parameter[0].split("=")]
                res[i] = (info[0],info[1],info[2],info[3],param_name,original_parameters[i].split("=")[-1].strip(),param_name)
            else:
                res[i] = (info[0],info[1],info[2],info[3],None,original_parameters[i].split("=")[-1].strip(),None)
    ...
    def get_replaced_body(self, filename, parameter_list, function_call_to_replace, variables_in_scope,global_init_lines,subs_not_to_inline,elim_lines):
        ...
        #Rename return var
        if is_function:
            return_var_name = f"{return_var}_return_value_{self.inline_num}"
            new_local_var_names[return_var] = return_var_name
        else:
        ...
        #  exit(0)
        #if original_subroutine_name == "calc_heat_cool_rtv":
        #  for line in res_lines:
        #    print(line)
        #  pexit("HMM")
        return ([line for line in lines if is_body_line(line)],(is_function,return_var_name),type)
        ...
        for line_index,line in enumerate(lines):
                ...
                while(len(sum_calls)>0 and modified_sum):
                    ...
                    if len(call["parameters"]) == 2 and call["parameters"][1] == "2" and len(param_dims) == 2 and param_dims[0] == global_subdomain_range_x:
                        ...
                        lines[line_index] = line
                        modified_sum = True
                    if len(call["parameters"]) == 2 and call["parameters"][1] == "3" and len(param_dims) == 3 and param_dims[0] == global_subdomain_range_x and param_dims[1] == "3":
                        line = self.replace_func_call(line,call,f"sum({call['parameters'][0]})")
                        lines[line_index] = line
                        modified_sum = True
        ...
        if len(indexes) == 0 and prof_type == "glob_n_vec":
          ...
          for dim in ["x", "y", "z"]:
            if prof_type == dim and var_dims in [[f"m{dim}__mod__cparam"],[":"]]:  
              res_index = f"[vertexIdx.{dim}]"
            elif prof_type == dim and var_dims == [f"n{dim}__mod__cparam"]:  
              res_index = f"[vertexIdx.{dim}-NGHOST_VAL]"
              ...
              res_index = f"[vertexIdx.{dim}]"
        elif prof_type == "x_vec" and indexes[0] == ":" and var_dims[0] == "nx__mod__cparam":
          res_index = f"[vertexIdx.x][{indexes[1]}-1]"
        elif segment[0] == "dline_1__mod__cdata":
          pexit("HMM: ",indexes)
        elif len(indexes) == 1:
    ...
    def gen_pushpars(self):
        ...
        for var in [var for var in self.static_variables if var not in self.static_variables_to_declare and var in contents]:
          ...
          if dims == ["3"] and type in ["logical","integer","real"]:
              ...
              res[mod][0].append(call)
          if type in ["logical","integer","real"] and len(dims) == 1 and dims != [":"] and dims != ["3"]:
              if remove_mod(dims[0]) == "-nghost:nghost":
                dims = ["2*nghost+1"]
              elif remove_mod(dims[0]) == "0:nprocx":
                dims = ["nprocx"]
              elif remove_mod(dims[0]) == "0:nprocy":
                dims = ["nprocy"]
              elif remove_mod(dims[0]) == "0:nprocz":
                dims = ["nprocz"]
              if mod not in res:
                  res[mod] = [[],0]
              res[mod][1] += 1
              if type == "real":
                res[mod][0].append(f"call copy_addr({name},p_par({res[mod][1]})) ! ({remove_mod(dims[0])})")
              elif type == "int":
    ...
    def transform_line_stencil(self,line,num_of_looped_dims, local_variables, rhs_var,vectors_to_replace, writes, loop_indexes):
        ...
        #TP: zero is here in case we use nopscalar.f90 many of the pscalar components go to zero
        #if "lpencil__mod__cparam" in line and "i_del2u__mod__cparam" in line and "else" in line:
        #  pexit("HMM",line) 
        ...
        if (
                ...
                if segment[0] == "df" or segment[0] == "f":
                  res = self.get_f_array_access(line,segment,local_variables,i,rhs_var,loop_indexes,writes)
                    ...
                    if len(var_dims) == 2 and var_dims[0] == "mx__mod__cparam" and var_dims[1] in bundle_dims and indexes[0] == "l1__mod__cparam:l2__mod__cdata" and ":" not in indexes[1]:
                      ...
                      res = f"{segment[0]}[vertexIdx.x-NGHOST][{indexes[1]}-1][{indexes[2]}-1]"
                        ...
                        if indexes[1] == "1":
                            ...
                            res = f"{segment[0]}[{indexes[2]}-1].y"
                        elif indexes[1] == "3":                
                            res = f"{segment[0]}[{indexes[2]}-1].z"
                    ...
                    #becomes a vector write for all of npscalar vals
                        ...
                        res = f"{segment[0]}[{indexes[-1]} -1]"
                    elif segment[0] in local_variables and  len(var_dims) == 3 and len(indexes) == 3 and var_dims[0] == global_subdomain_range_x and var_dims[1] in bundle_dims and var_dims[2] in bundle_dims and indexes[0] == ":" and ":" not in indexes[1] and ":" not in indexes[2]:
                        res = f"{segment[0]}[{indexes[1]} -1][{indexes[2]} -1]"
                    #becomes a vector to vector indexes
                        ...
                        if(indexes[-1] == ":"):
                            pexit("Not supported at the moment!\n",line)
                        res = f"{segment[0]}[{indexes[-1]} -1].x"
                        ...
                        res = f"{segment[0]}[{indexes[-1]} -1]"
                    elif segment[0] in local_variables and var_dims[:-1] == [global_subdomain_range_x,"3","3"] and var_dims[-1] in bundle_dims and indexes == []:
                        res = f"{segment[0]}"
                    ...
                    #AcTensor
                      ...
                      res = line[segment[1]:segment[2]]
                    elif rhs_var is None and len(indexes) == 0: 
                      res = line[segment[1]:segment[2]]
                    elif len(var_dims) == 1 and len(indexes) == 1:
            ...
            if "%" in res_line or res_line.strip() == "return":
        ...
        if (num_of_looped_dims==0 or num_of_looped_dims == 1 or num_of_looped_dims==2 or num_of_looped_dims is None) and (rhs_var in local_variables or rhs_var == "f" or rhs_var is None): 
            for i in range(len(array_segments_indexes)):
                ...
                #comes from spread statement
                    ...
                    #TP: loop across nx is okay since we pad the array
                ...
                elif (len(var_dims) == 2 and indexes == [":",":"] and segment[0] in local_variables) or ((len(indexes) == 2 and num_of_looped_dims == 2 and indexes[0] == "l1__mod__cparam:l2__mod__cdata" and indexes[1] == ":" and var in local_variables)):
            ...
            res_line = res_line + line[last_index:]
            res_line = res_line
            if ":" in res_line or "explicit_index" in res_line or not has_balanced_parens(res_line) or "()" in res_line:
        ...
        if (num_of_looped_dims==0 or num_of_looped_dims==2) and (rhs_var in local_variables or rhs_var == "f"): 
            for i in range(len(array_segments_indexes)):
                ...
                if segment[0] == "f":
                    ...
                    vtxbuf_name = self.get_vtxbuf_name_from_index("VTXBUF_", indexes[-1])
    ...
    def transform_init_line(self,line,i,local_variables,orig_params):
        vars_in_line = {}
        ...
        if dims[:-1] ==  [global_subdomain_range_x,"3"] and dims[-1] in bundle_dims:
            ...
            return res
        if (dims[:-1] ==  [global_subdomain_range_x] and dims[-1] in bundle_dims) or (len(dims) == 1 and dims[0] in bundle_dims):
            #tensors are not yet supported
            res = ""
            for var in vars_to_declare:
            ...
            if rhs_indexes == [] or (rhs_var == "f" and rhs_indexes[:-1] == [":",":",":"] and ":" not in rhs_indexes[-1]):
                ...
                #not really a spread
                    ...
                    if(len(param_info[3]) == 1 and param_info[3][0] == rhs_info[3][0] and rhs_info[3][1] == call["parameters"][2]):
                       ...
                       res_lines.append(res_line)
                    ##TP: hack
                    elif len(param_info[3]) == 1 and rhs_var == "f" and param_info[3][0] in ["nx__mod__cparam","mx__mod__cparam"] and call["parameters"][2] == "my__mod__cparam":
                       #print("SAFE TO REMOVE SPREAD")
                       res_line = self.replace_func_call(line,call,call["parameters"][0])
                       #print("RES: ",res_line)
                       res_lines.append(res_line)
    ...
    def transform_any_calls(self,lines,local_variables, variables):
        for line_index,line in enumerate(lines):
                ...
                if len(any_calls) == 1 and len(other_calls) == 0:
                    while(len(any_calls)>0 and keep_doing):
                        call = any_calls[0]
                        ...
                        if(len(parts) == 2):
                            ...
                            #TP: for bundle dims do not do anything
    ...
    def transform_tiny(self,lines,local_variables,variables):
        for line_index,line in enumerate(lines):
            for call in [x for x in self.get_function_calls_in_line(line,variables) if x["function_name"] == "tiny"]:
                ...
                if type == "real":
                    line = self.replace_func_call(line,call,"AC_REAL_MIN")
                    lines[line_index] = line
                else:
    ...
    def get_pointer_target(self,pointer_in):
            ...
            if(len(possible_modules) != 1):
              ...
              if self.offloading and pointer_in in self.static_variables:
                if self.static_variables[pointer_in]["dims"] == [] and self.static_variables[pointer_in]["type"] == "real":
                    return "0.0"
    ...
    def expand_function_call_main(self, subroutine_lines, subroutine_name, filename, calls_to_expand,global_init_lines,sub_modules,subs_not_to_inline,elim_lines,local_variables):
        ...
        replaced_function_call_lines,(is_function,return_var_name),res_type = self.expand_function_call(subroutine_lines, subroutine_name, filename, call_to_expand,local_variables,global_init_lines,subs_not_to_inline,elim_lines,local_variables)
        ...
        global_init_lines = unique_list(global_init_lines)
        if is_function:
            subroutine_lines = subroutine_lines[:1] +  ["use {module}" for module in sub_modules] + global_init_lines + [f"{res_type} :: {return_var_name}"] + subroutine_lines[1:]
        else:
        ...
        for line_num in range(len(subroutine_lines)):
          if not has_replaced_call:
            ...
            if line == call_to_expand["line"] or f"{call_to_expand['function_name']}(" in line or f"call {call_to_expand['function_name']}" == line and "print*" not in line:
                ...
                func_calls = [call for call in self.get_function_calls_in_line(subroutine_lines[line_num],local_variables) if call["function_name"] == call_to_expand["function_name"]]
                if len(func_calls) == 0:
                  continue
                func_call = func_calls[0]
                ...
                if is_function:
                    ...
                    new_name = return_var_name
                    subroutine_lines[line_num] = self.replace_func_call(subroutine_lines[line_num], func_call, new_name) 
        ...
        for line in lines:
                if line == call_to_expand["line"] or f"{call_to_expand['function_name']}(" in line:
                    if not(line == call_to_expand["line"] or f"{call_to_expand['function_name']}(" in line):
                        pexit("WRONG!")
                    function_calls = [call for call in self.get_function_calls_in_line(line,local_variables) if call["function_name"] == call_to_expand["function_name"]]
    ...
    def normalize_where_calls(self,lines,local_variables):
      ...
      for line_index, line in enumerate(lines):
        ...
        if "where" in line and "(" in line:
          ...
          if len(where_calls) == 1 and line[where_calls[0]["range"][1]:-1].strip() != "":
    ...
    def normalize_result(self,lines):
        local_variables = {parameter:v for parameter,v in self.get_variables(lines, {},self.file,True).items() }
    ...
    def inline_all_function_calls(self,filename,subroutine_name,new_lines,subs_not_to_inline=None,elim_lines = True):
        ...
        #normalize .eq. => == and .ne. => /=
        #.lt. => < and .gt. => >
        new_lines = self.normalize([x.replace(".eq.","==").replace(".ne.","/=").replace(".lt.","<").replace(".gt.",">")  for x in new_lines])
...
def main():
    ...
    parser.safe_subs_to_remove.append("after_boundary")
    ...
    if config["offload"]:
        ...
        parser.ignored_subroutines.extend(["xymax_mn_name_z","sum_mn_name","max_mn_name","yzsum_mn_name_x","xzsum_mn_name_y","xysum_mn_name_z","zsum_mn_name_xy","ysum_mn_name_xz","phizsum_mn_name_r","phisum_mn_name_rz","integrate_mn_name","sum_lim_mn_name","save_name"])
        ...
        if not os.path.isfile("res-inlined.txt"):
            ...
            new_lines = parser.func_info[subroutine_name]["inlined_lines"][filename]
            new_lines = parser.normalize_result(new_lines)
            file = open("res-inlined.txt","w")
            for line in new_lines:
                file.write(f"{line}\n")
            file.close()
        ...
        for line_index, line in enumerate(new_lines):
                ...
                where_func_calls = [call for call in func_calls if call["function_name"] in ["where","elsewhere"]]
                where_call_segments = [(None, call["range"][0], call["range"][1]) for call in where_func_calls]
                where_map_vals = []
                for call in where_func_calls:
                  is_scalar_if = False
                  arr_segs = parser.get_array_segments_in_line(line,variables)
                  struct_segs = parser.get_struct_segments_in_line(line,variables)
                  for seg in arr_segs:
                ...
                if line.strip() == "endwhere":
                  new_lines[line_index] = "endif"
                elif line.strip() == "elsewhere":
                  new_lines[line_index] = "else"
        new_lines = parser.eliminate_while(new_lines)
        for i,line in enumerate(new_lines):
          if "elsewhere" in line:
            print("Was not able eliminate this elsewhere: ",line,new_lines[i+1])
            pexit("WRONG\n");
        local_variables = {parameter:v for parameter,v in parser.get_variables(new_lines, {},filename,True).items() }
        ...
        if(not gen_only_vars):
            ...
            for var in parser.static_variables_to_declare: 
              ...
              if dims == ["nx__mod__cparam","3"]:
                file.write(f"{type}3 {name}\n")
              elif len(dims) == 2 and dims[0]  == "nx__mod__cparam" and dims[1] in bundle_dims:
